...
class WindFarmEnv(Env):
    ...
    def __init__(
        ...
        # Initialize last action to zero
        self.last_action = np.zeros(len(self.turbines))
    ...
    def step(self, action):
        ...
        self.state = self._get_state()
        self.floris_interface.calculate_wake()
        #print(self.floris_interface.get_turbine_power())
    ...
    def MAstep(self, action):
        ...
        self.floris_interface.calculate_wake()
        reward = self.floris_interface.get_turbine_power()
        if reward == None:
            reward = [0 for _ in range(self.n_turbines) ]
        reward = [ 0 if np.isnan(x) else x * self._reward_scaling_factor for x in reward]
        self.last_action = action
        return self.state, reward, done, {}
    ...
    def get_env_info(self) -> dict:
        map_info = {
            'n_agents': self.n_turbines,
            'agents_name': self.turbines,
            'obs_space': self.observation_space,
            'state_space': self.observation_space,
            'obs_shape': 3, #len(self.observed_variables),
            'state_shape': 3, # len(self.state),
            'n_actions': 1
        }
        return map_info
    ...
    def get_obs(self):
        if self._has_states:
            ...
            if len(self.current_flow_points[0]) > 0:
                self._current_flow = self.floris_interface.get_set_of_points(*self.current_flow_points)
            state = [[] for _ in range(self.n_turbines)]
            for d in self.observed_variables:
                state[d['index']].append(self._get_measurement_point_data(d))
            # inject noise
            if self._perturbed_observations is not None:
                added_noise = np.zeros_like(state)
                added_noise.put(self._perturbed_observations, self._noise.step()['value'], mode='raise')
            ...
            # rescale and clip off
            lowpt = [[] for _ in range(self.n_turbines)]
            for d in self.observed_variables:
                lowpt[d['index']].append(d['min'])

            highpt = [[] for _ in range(self.n_turbines)]
            for d in self.observed_variables:
                highpt[d['index']].append(d['max'])
            state_delta_pt = np.array(highpt) - np.array(lowpt)
            if self._normalize_observations:
                state = (np.array(state) - lowpt) / state_delta_pt
                state = np.clip(state, np.zeros_like(lowpt), np.ones_like(highpt))
            else:
                state = np.clip(state, lowpt, highpt)
            return list(state)
        else:
            return 0
        
    def get_avail_actions(self):
        action_space = [[0] for _ in range(self.n_turbines)]
        print(action_space)
        return action_space
